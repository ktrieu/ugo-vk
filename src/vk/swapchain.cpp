...
Swapchain::Swapchain(VulkanContext &context, Window &window) : _context(context)
{
    ...
    VkSurfaceCapabilitiesKHR caps = _context.get_device().physical_device().get_surface_caps();
    ...
    uint32_t graphics_family = _context.get_device().graphics_family();
    uint32_t present_family = _context.get_device().present_family();
    ...
    auto result = vkCreateSwapchainKHR(_context.get_device().device(), &info, nullptr, &_swapchain);
    vk_check(result);

    uint32_t image_count;
    result = vkGetSwapchainImagesKHR(_context.get_device().device(), _swapchain, &image_count, nullptr);
    vk_check(result);

    _images.resize(image_count);
    result = vkGetSwapchainImagesKHR(_context.get_device().device(), _swapchain, &image_count, _images.data());
    vk_check(result);
...
void Swapchain::destroy()
{
    ...
    {
        vkDestroyImageView(_context.get_device().device(), view, nullptr);
    }

    vkDestroySwapchainKHR(_context.get_device().device(), _swapchain, nullptr);
}

uint32_t Swapchain::acquire_image(VkSemaphore completion)
{
    uint32_t image_idx;
    auto result = vkAcquireNextImageKHR(_context.get_device().device(), _swapchain, 1000000000, completion, nullptr, &image_idx);
    vk_check(result);
...
{
    auto formats = _context.get_device().physical_device().get_surface_formats();
...
{
    auto modes = _context.get_device().physical_device().get_present_modes();
...
{
    auto caps = _context.get_device().physical_device().get_surface_caps();
...
VkImageView Swapchain::create_image_view(VkImage image)
{
    ...
    VkImageView view;
    auto result = vkCreateImageView(_context.get_device().device(), &info, nullptr, &view);
    vk_check(result);
